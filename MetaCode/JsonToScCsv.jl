...
function Prolog()
      ...
      write(inpfile,"<<<<<<<<<<<<<<<<<<  SC Outputs for 42  >>>>>>>>>>>>>>>>>>>\n");
...
function AddToHeaderString(H,StructName,StructSize,SubstructName,SubstructSize,
   VarName,VarSize)
      ...
      H *= " "^Indent*"fprintf(outfile[Isc],\",Sc_";
      H *= StructName;
      if (HasK) H *= "%ld_"; end
      H *= VarName
      if (HasI) H *= "_%ld"; end
...
function AddToDataString(D,StructName,StructSize,SubstructName,SubstructSize,
   VarName,VarSize,VarType)
      ...
      D *= " "^Indent*"fprintf(outfile[Isc],\","*fmt*"\",S->";
      D *= StructName;
      if (HasK) D *= "[k]"; end
      if (StructName != "") D *= "."; end
...
function AddToFunctionString(F,Name,HeaderString,DataString)
      ...
      F *= "            if (SC[Isc].Exists) {\n";
      F *= "               S = &SC[Isc];\n";
      F *= "               fprintf(outfile[Isc],\"%18.12le\",SimTime);\n";
      F *= DataString;
      F *= "               fprintf(outfile[Isc],\"\\n\");\n"
      F *= "            }\n";
      F *= "         }\n";
...
function BeginMainFunction()
      ...
      write(cfile,"/"*"*"^78*"/\n");
      write(cfile,"void WriteScVarsToCsv(void)\n{\n");
      ...
      write(cfile,"         First = 0;\n\n");
      write(cfile,"         infile = FileOpen(InOutPath,\"Inp_ScOutput.txt\",\"r\");\n");
...
function main()
      ...
      global cfile = open("./Source/AutoCode/WriteScToCsv.c","w");
      global inpfile = open("./MetaCode/Inp_ScOutput.txt","w");
      
      DeclareString = "long ScTlmEnabled, MaxScCtr;\n"
      CallString = "      if (ScTlmEnabled) WriteScToCsv();\n";
      EnableString = "TRUE  0.25   ! Sc Tlm Enabled, Interval\n";
      FunctionString = "";
      FirstString = "";
...
# ... Find S struct
      Isc = 0;
      for It in 1:Ntbl
         if Tbl[It]["Table Name"] == "SCType";
            Isc = It;
         end
      end
      Sc = Tbl[Isc]["Table Data"];
      Nv = length(Sc);
...
# ... Loop over Outputs
      ...
      # Write Function
      FunctionString = AddToFunctionString(FunctionString,"Sc",HeaderString,
         DataString);
      FirstString = AddToFirstString(FirstString,"Sc");
      
# ... Loop over structs within Sc
      for Iv=1:Nv
         HeaderString = "";
         DataString = "";
         Struct = Sc[Iv];
         if Struct["I/O Role"] == "STRUCT"
            ...
            if (Is > 0)
               ...
               if (HasInOut)
                  ...
                  DeclareString *= "long Sc$(Name)TlmEnabled, MaxSc$(Name)Ctr;\n";
                  # Build Enable string
                  EnableString *= "FALSE  5.0   ! Sc$(Name) Tlm Enabled, Interval\n";
                  # Build Call string
                  CallString *= "      if (Sc$(Name)TlmEnabled) WriteSc$(Name)ToCsv();\n"
                  # Write Function
                  FunctionString = AddToFunctionString(FunctionString,"Sc$(Name)",
                     HeaderString,DataString);
                  FirstString = AddToFirstString(FirstString,"Sc$(Name)");
               end
                     ...
                     if (Iss > 0)
                        ...
                        for Ivvv=1:Nvvv
                           ...
                           if (V["I/O Role"] == "INPUT" || 
                              ...
                              DataString = AddToDataString(DataString,
                                 StructName,StructSize,SubstructName,
                        ...
                        if (HasInOut)
                           ...
                           CallString *= "      if (Sc$(Name)TlmEnabled) WriteSc$(Name)ToCsv();\n"
                           # Write Function
                           FunctionString = AddToFunctionString(FunctionString,"Sc$(Name)",HeaderString,DataString);
                           FirstString = AddToFirstString(FirstString,"Sc$(Name)");
                        end
                     end
                  end
...
################################################################################
# Run this file if called from command line, like "julia JsonToScCsv.jl"
